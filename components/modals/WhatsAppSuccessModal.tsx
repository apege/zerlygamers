  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl border-2 border-emerald-300 w-full max-w-lg p-5 sm:p-7 shadow-2xl relative overflow-hidden flex flex-col gap-4 text-left max-h-[92vh] overflow-y-auto">
        {/* Background glow */}
        <div aria-hidden="true" className="absolute -top-12 -right-12 w-36 h-36 bg-emerald-300/20 rounded-full blur-xl pointer-events-none" />

        {/* Close Button */}
        <button
          type="button"
          onClick={onClose}
          aria-label="Tutup konfirmasi pesanan WhatsApp"
          className="absolute top-4 right-4 text-gray-500 hover:text-emerald-600 transition-colors p-1.5 rounded-full hover:bg-emerald-50 cursor-pointer z-10"
        >
        ...
        <div className="flex flex-col items-center text-center gap-2 pt-2">
          <div aria-hidden="true" className="w-16 h-16 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-500 text-white flex items-center justify-center shadow-lg shadow-emerald-500/25 animate-bounce">
            <MessageCircle className="w-8 h-8 fill-white/20" />
          </div>

          <div className="space-y-1">
            <div className="inline-flex items-center gap-1 text-[11px] font-black uppercase tracking-wider text-emerald-700">
              <Sparkles aria-hidden="true" className="w-3.5 h-3.5" />
              <span>Pesanan Siap Dikirim ke WhatsApp</span>
            </div>
            <h3 id="whatsapp-modal-title" className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight">
              Lanjutkan Order di WhatsApp! 💬
            </h3>
            <p className="text-xs sm:text-sm text-gray-700 font-medium max-w-sm mx-auto leading-relaxed">
              Format pesanan telah dibuat. Silakan kirimkan chat ke Admin <strong className="text-emerald-800">{storeName}</strong> untuk menyelesaikan order.
            </p>
          </div>
        </div>
        ...
        <div className="bg-emerald-50/70 border border-emerald-200 rounded-2xl p-4 flex flex-col gap-2.5 text-xs">
          ...
          <div className="flex justify-between items-center py-1">
            <span className="text-gray-600 font-medium">Metode Pemesanan</span>
        ...
        <div className="flex flex-col gap-2 pt-1">
          ...
          >
            <MessageCircle aria-hidden="true" className="w-4 h-4 fill-white/20" />
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full py-2.5 px-4 rounded-xl text-gray-600 hover:text-gray-900 hover:bg-gray-100 font-bold text-xs transition-colors cursor-pointer"
          >
